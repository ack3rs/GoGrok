...
func SetUpEnv() {
	...
	if GetEnvString("DSN", "DSN") == "DSN" {
		l.Info("Local DSN not set getting from AWS Secrets Manager")

		SecretName := GetEnvString("SECRET_NAME", "prod/propertyassistantweb")
		l.With("SecretName", SecretName).Debug("Getting AWS Secrets")

		// Get the AWS Secrets
		m, err := godotenv.Unmarshal(GetAWS(SecretName))
		if err != nil {
			l.With("Error", err).Debug("Error loading AWS config")
			return
		}
		for key, val := range m {
			l.With("key", key).Info("Setting ENV")
			_ = os.Setenv(key, val)
		}
	}
}
